...
<body class="bg-[#f8fafc] min-h-screen flex flex-col pt-[60px] lg:pt-[80px]">
    ...
    <div class="navbar-outer">
        ...
        {{-- Logo + Title --}}
        <a href="{{ route('bod.dashboard') }}" class="flex items-center gap-2 lg:gap-4 flex-shrink-0 hover:opacity-90 transition-opacity">
        ...
        <div class="flex items-center ml-auto lg:pr-6 pr-0 space-x-6 lg:space-x-8">
            ...
            {{-- Notification --}}
            @php
                $unreadNotifications = isset($notifications) && $notifications ? (is_array($notifications) ? collect($notifications)->where('is_read', false) : $notifications->where('is_read', false)) : collect();
            ...
            <div class="relative" id="bell-wrapper">
                ...
                <div id="bell-dropdown" class="dropdown-panel hidden absolute right-0 mt-3 w-[320px] bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-50">
                    ...
                    @if($hasUnreadNotif)
                        <ul class="divide-y divide-gray-50 max-h-60 overflow-y-auto">
                            @foreach($unreadNotifications->take(3) as $notif)
                                <li class="px-4 py-3 flex items-start gap-3 hover:bg-gray-50 transition-colors cursor-pointer" onclick="window.location='{{ route('bod.notifikasi') }}'">
                    ...
                    <div class="px-5 py-3.5 bg-white border-t border-gray-100 text-center">
                        <a href="{{ route('bod.notifikasi') }}" class="text-[13px] font-bold text-gray-400 hover:text-gray-600 transition-colors">Lihat semua notifikasi</a>
                    </div>
                </div>
            </div>

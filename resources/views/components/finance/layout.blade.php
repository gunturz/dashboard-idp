...
<body class="bg-[#f8fafc] min-h-screen pt-[60px] lg:pt-[80px]">
    ...
    <div class="navbar-outer">
        ...
        <div class="flex items-center space-x-2 sm:space-x-3 pl-0 lg:pl-4 border-l-0 lg:border-l border-white/20 ml-auto lg:pr-6 pr-0">
                ...
                <div id="mobile-menu-dropdown" class="dropdown-panel hidden absolute right-0 mt-3 w-[300px] bg-white rounded-[1.25rem] shadow-[0_15px_40px_-10px_rgba(0,0,0,0.15)] border border-gray-100 overflow-hidden z-50 origin-top-right">
                    ...
                    <div class="px-5 py-5 border-b border-gray-100 flex items-center justify-between bg-white relative">
                        <div class="flex items-center gap-3.5">
                            @php
                                $nameParts = explode(' ', $user->nama ?? $user->name ?? 'User Name');
                                $initials = count($nameParts) >= 2 ? strtoupper(substr($nameParts[0], 0, 1) . substr($nameParts[1], 0, 1)) : strtoupper(substr($nameParts[0], 0, 2));
                            @endphp
                            <div class="w-[52px] h-[52px] rounded-full bg-[#343E4E] text-white flex items-center justify-center font-bold text-lg tracking-wide outline outline-1 outline-[#343E4E]/20 ring-[3px] ring-white shadow-sm flex-shrink-0">
                                {{ $initials }}
                            </div>
                            <div class="flex flex-col">
                                <span class="text-[13px] font-bold text-[#343E4E] uppercase tracking-[0.02em] leading-snug break-words line-clamp-2 max-w-[130px]">{{ $user->nama ?? $user->name ?? 'User Name' }}</span>
                    ...
                    <ul class="py-3 px-3">
                        <li class="mb-1">
                            <a href="{{ route('finance.dashboard') }}" class="mobile-nav-link flex items-center w-full px-4 py-3 rounded-xl text-[14px] transition-colors whitespace-nowrap {{ request()->routeIs('finance.dashboard') ? 'active' : '' }}">
                                Dashboard Finance
                            </a>
                        </li>
                        <li class="mb-1">
                            <a href="{{ route('finance.permintaan_validasi') }}" class="mobile-nav-link flex items-center w-full px-4 py-3 rounded-xl text-[14px] transition-colors whitespace-nowrap {{ request()->routeIs('finance.permintaan_validasi') ? 'active' : '' }}">
                                Permintaan Validasi
                            </a>
                        </li>

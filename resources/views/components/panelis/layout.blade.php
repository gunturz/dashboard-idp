...
<body class="bg-[#ffffff] min-h-screen flex flex-col pt-[60px] lg:pt-[80px] relative overflow-x-hidden">
    ...
    <div class="navbar-outer">
        ...
        {{-- Logo + Title --}}
        <a href="{{ route('panelis.dashboard') }}" class="nav-logo">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function panelisShowToast(title, desc) {
                ...
                toast.innerHTML = `
                    <div style="flex-shrink:0;width:38px;height:38px;border-radius:50%;background:linear-gradient(135deg,#ede9fe,#ddd6fe);display:flex;align-items:center;justify-content:center;">
                        <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='#7c3aed' style='width:20px;height:20px;'>
                    ...
                    <div style="position:absolute;bottom:0;left:0;height:3px;background:linear-gradient(90deg,#8b5cf6,#a78bfa);width:100%;transform-origin:left;animation:panelisBarShrink 5s linear forwards;border-radius:0 0 0 14px;"></div>
                `;

                if (!document.getElementById('panelis-toast-style')) {
                    let s = document.createElement('style');
                    s.id = 'panelis-toast-style';
                    s.textContent = '@keyframes panelisBarShrink{from{transform:scaleX(1)}to{transform:scaleX(0)}}';

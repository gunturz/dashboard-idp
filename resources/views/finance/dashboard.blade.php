<x-finance.layout title="Dashboard Finance" :user="$user">
    ...
    <div class="dash-header animate-title">
        ...
        {{-- Notification Alert --}}
        <div class="bg-amber-50 border border-amber-200 rounded-2xl p-5 flex flex-col sm:flex-row items-center justify-between shadow-sm mb-8">
            <div class="flex items-center gap-4 mb-3 sm:mb-0">
                ...
                <p class="text-amber-900 font-medium text-sm">
                    Ada <span class="font-bold text-amber-600">{{ $pending }} Permintaan</span> yang menunggu validasi anda
                </p>
            </div>
                        ...
                        <tr>
                            <td class="text-left px-6">
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-[11px] text-gray-500 italic leading-tight mt-1">{{ $project->talent->position->position_name ?? '-' }} &rarr; {{ $project->talent->promotion_plan->targetPosition->position_name ?? '?' }}</span>
                                    <span class="text-[11px] text-gray-500 italic">{{ $project->talent->department->nama_department ?? '-' }}</span>
                                </div>
                            </td>

<div>
    ...
    @if($activeSection === 'kompetensi')
        <div class="w-full animate-fade-in">
            ...
            <div class="section-title">Heatmap Kompetensi</div>
            <div class="legend">
                            ...
                            <tr>
                                ...
                                @foreach ($talents as $talent)
                                    @php
                                        ...
                                        $scoreTalent = $detail->score_talent ?? 0; $scoreAtasan = $detail->score_atasan ?? 0;
                                        $gap = $detail->gap_score ?? 0; $finalScore = ($scoreTalent + $scoreAtasan) / 2;
                                    ...
                                    <tr class="border-b border-gray-100 hover:bg-slate-50/50 transition duration-150">
                                        ...
                                        <td class="py-4 px-6 text-center">
                                            <div class="inline-flex items-center gap-2 bg-gray-50 border border-gray-200 px-3 py-1.5 rounded-full">
                                                <div class="w-2.5 h-2.5 rounded-full {{ in_array($proj->status, ['Verified', 'Approved']) ? 'bg-emerald-500' : 'bg-amber-500' }}"></div>
                                                <span class="font-bold text-xs uppercase text-slate-700">{{ in_array($proj->status, ['Verified', 'Approved']) ? 'Approve' : ($proj->status ?: 'Pending') }}</span>
                                            </div>
                                        </td>
                                    </tr>
                                @empty
                                                    ...
                                                    <div class="p-4 bg-slate-50 border border-slate-200 rounded-xl">
                                                        ...
                                                        @php
                                                            ...
                                                            if($act->document_path){
                                                                if(str_starts_with($act->document_path, '["')) { $dPaths = json_decode($act->document_path, true); $dNames = explode(', ', $act->file_name); } 
                                                                else { $dPaths = [$act->document_path]; $dNames = [$act->file_name]; }
                                                            }
                                                        @endphp
                                                        @if(count($dPaths) > 0)

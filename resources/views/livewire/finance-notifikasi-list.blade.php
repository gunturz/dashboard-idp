{{--
    Livewire Component: finance-notifikasi-list
    Wire:poll setiap 10 detik sebagai fallback.
    Desain identik dengan PDC Admin notifikasi list.
--}}
<div wire:poll.10s="loadNotifications" x-data @app-notification-received.window="$wire.loadNotifications()">
    ...
    <div class="page-header animate-title" style="flex-wrap: wrap;">
        ...
        <div>
            ...
            <div class="page-header-sub">Informasi terbaru terkait validasi IDP</div>
        </div>
        <div class="page-header-actions flex-wrap" style="margin-top: 0;">
            @if(count($notifications) > 0)
                @if($unreadCount > 0)
                    <button type="button" wire:click="markAllRead" class="btn-prem btn-teal whitespace-nowrap" style="padding: 8px 16px; font-size: 0.85rem;">
                    ...
                    @else
                        <div class="flex flex-col sm:flex-row sm:items-center gap-2 w-full sm:w-auto">
                            ...
                            <div class="flex items-center gap-2 w-full sm:w-auto">
                                <button wire:click="selectAll" class="btn-prem btn-blue flex-1 sm:flex-initial" style="padding: 8px 16px; font-size: 0.85rem; background: #3b82f6; color: white;">
                                    Pilih Semua
                                </button>
                                <button wire:click="toggleEditMode" class="btn-prem btn-ghost flex-1 sm:flex-initial" style="padding: 8px 16px; font-size: 0.85rem;">
                                    Batal
                                </button>
                                <button wire:click="deleteSelected" class="btn-prem btn-red flex items-center justify-center gap-2 flex-1 sm:flex-initial" style="padding: 8px 16px; font-size: 0.85rem; {{ empty($selectedNotifications) ? 'opacity: 0.5; cursor: not-allowed;' : '' }}" @if(empty($selectedNotifications)) disabled @endif>
        ...
        <div style="padding: 16px 20px; display: flex; flex-direction: column; gap: 10px;">
            ...
            @empty
                <div class="empty-prem" style="border:none; padding:48px 20px;">
                    ...
                    <p>Anda akan menerima pemberitahuan di sini saat PDC Admin mengirim validasi.</p>
                </div>
            @endforelse
        </div>
    </div>

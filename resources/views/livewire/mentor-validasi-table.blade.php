<div>
    ...
    <div class="inline-flex items-center gap-1.5 mb-8 bg-slate-100/80 p-1.5 rounded-full border border-slate-200/60 overflow-x-auto custom-scrollbar w-fit">
        <button wire:click="switchTab('exposure')" class="btn-prem {{ $activeTab == 'exposure' ? 'btn-dark active px-6 rounded-full' : 'btn-ghost px-6 rounded-full border-transparent bg-transparent hover:bg-slate-200/80' }}">Exposure</button>
        <button wire:click="switchTab('mentoring')" class="btn-prem {{ $activeTab == 'mentoring' ? 'btn-dark active px-6 rounded-full' : 'btn-ghost px-6 rounded-full border-transparent bg-transparent hover:bg-slate-200/80' }}">Mentoring</button>
        <button wire:click="switchTab('learning')" class="btn-prem {{ $activeTab == 'learning' ? 'btn-dark active px-6 rounded-full' : 'btn-ghost px-6 rounded-full border-transparent bg-transparent hover:bg-slate-200/80' }}">Learning</button>
    </div>
        ...
        <div class="p-0 overflow-x-auto custom-scrollbar">
            <table class="highlight-table mb-0">
                ...
                <tbody>
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="py-8 text-center text-gray-400 text-sm italic">Belum ada aktivitas {{ ucfirst($activeTab) }}.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>

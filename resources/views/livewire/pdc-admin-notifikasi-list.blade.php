...
<div wire:poll.10s="loadNotifications" x-data @app-notification-received.window="$wire.loadNotifications()">
    ...
    <div class="prem-card w-full mb-6">
        <div class="prem-card-header flex justify-between items-center">
            <span class="prem-card-title">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                    style="width: 18px; height: 18px; color: #14b8a6; flex-shrink: 0;">
                    <path
                        d="M10 2a6 6 0 00-6 6v3.586l-.707.707A1 1 0 004 14h12a1 1 0 00.707-1.707L16 11.586V8a6 6 0 00-6-6z" />
                    <path d="M10 18a3 3 0 01-3-3h6a3 3 0 01-3 3z" />
                </svg>
        ...
        <div style="padding: 16px 20px; display: flex; flex-direction: column; gap: 10px;">
            @forelse ($notifications as $notif)
                <label class="notif-item {{ !$notif['is_read'] ? 'unread' : '' }} type-{{ $notif['type'] ?? 'info' }}"
                    ...
                    <div style="flex:1; min-width:0;">
                        <div
                            style="display:flex; justify-content:space-between; align-items:flex-start; gap:10px; margin-bottom:4px;">
                            <h3 style="font-weight:700; color:#1e293b; font-size:.9rem; margin:0; flex:1;">
                                {!! $notif['title'] !!}
                            </h3>
                            @if ($notif['badge'] ?? false)
                                <span class="badge"
                                    style="flex-shrink:0; background: {{ $notif['badge_bg'] ?? '#eff6ff' }}; color: {{ $notif['badge_color'] ?? '#1d4ed8' }};">{{ $notif['badge'] }}</span>
                            @endif
                            @if (!$notif['is_read'])
                                <div class="notif-unread-dot"></div>
                            @endif
                        </div>
                        <p style="font-size:.83rem; color:#475569; margin:0 0 6px; line-height:1.6;">{!! $notif['desc'] !!}
                        </p>
            ...
            @empty
                <div class="empty-prem" style="border:none; padding:48px 20px;">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                        style="width: 56px; height: 56px; color: #cbd5e1;">
                        <path
                            d="M10 2a6 6 0 00-6 6v3.586l-.707.707A1 1 0 004 14h12a1 1 0 00.707-1.707L16 11.586V8a6 6 0 00-6-6z" />
                        <path d="M10 18a3 3 0 01-3-3h6a3 3 0 01-3 3z" />
                    </svg>
    ...
    @if($isEditMode)
        <div
            class="fixed bottom-6 right-6 z-50 flex items-center gap-3 bg-white px-5 py-3 rounded-full shadow-lg border border-gray-100 animate-title">
            <span class="text-sm font-semibold text-gray-600 mr-2">{{ count($selectedNotifications) }} dipilih</span>
            <button wire:click="toggleEditMode" class="btn-prem btn-ghost"
                style="padding: 6px 14px; border-radius: 99px;">Batal</button>
            <button wire:click="deleteSelected" class="btn-prem btn-red" style="padding: 6px 14px; border-radius: 99px;"
                @if(empty($selectedNotifications)) disabled style="opacity: 0.5; cursor: not-allowed;" @endif>

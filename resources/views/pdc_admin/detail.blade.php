<x-pdc_admin.layout title="Detail Progress Talent – Individual Development Plan" :user="$user">
    ...
    <div class="mb-6 animate-title">
        ...
        <div class="pill-nav-wrapper mb-6">
            <a href="javascript:void(0)" onclick="handleGlobalBack(event, '{{ route('pdc_admin.progress_talent') }}')"
                class="pill-nav-back" title="Kembali">
            ...
            <div class="pill-nav-tabs">
                <div class="pill-tab active" id="pill-kompetensi" onclick="switchSection('kompetensi', this)">Kompetensi
                </div>
                <div class="pill-tab" id="pill-idp" onclick="switchSection('idp', this)">IDP Monitoring</div>
                <div class="pill-tab" id="pill-project" onclick="switchSection('project', this)">Project Improvement
                </div>
            </div>
        </div>
    ...
    <div id="section-kompetensi">
                    ...
                    <div class="donut-container">
                        ...
                        @foreach($charts as $chart)
                            ...
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <button class="donut-label-btn" style="background-color: {{ $chart['color'] }};"
                                    onclick="switchToLogbook({{ $typeId }})" title="Lihat logbook {{ $chart['label'] }}">
                                    {{ $chart['label'] }}
        ...
        <script>
            ...
            function handleGlobalBack(e, fallbackUrl) {
                e.preventDefault();
                const logbookSection = document.getElementById('section-logbook');
                // Jika sedang melihat logbook, kembali ke IDP
                if (logbookSection && !logbookSection.classList.contains('hidden')) {
                    switchSection('idp', document.getElementById('pill-idp'));
                } else {
                    // Jika di section lain, kembali ke halaman sebelumnya
            ...
            function switchSection(targetId, el) {
                // All known sections
                const sectionIds = ['kompetensi', 'idp', 'project', 'logbook'];
                ...
                if (el && el.classList.contains('pill-tab')) {
                    el.classList.add('active');
                } else {
                    // Find pill tab by id mapping
                    const pillMap = { 'kompetensi': 'pill-kompetensi', 'idp': 'pill-idp', 'project': 'pill-project', 'logbook': 'pill-idp' };
                    const pillId = pillMap[targetId];
                    if (pillId) {
            ...
            function switchToLogbook(typeId) {
                // Show logbook section
                switchSection('logbook', null);
                // Activate the IDP Monitoring pill since logbook is linked from IDP
                document.querySelectorAll('.pill-tab').forEach(tab => tab.classList.remove('active'));
                const idpPill = document.getElementById('pill-idp');
                if (idpPill) idpPill.classList.add('active');
                // Filter to the correct type
                const targetPill = document.querySelector('.pill[onclick="filterLog(' + typeId + ', this)"]');
                if (targetPill) filterLog(typeId, targetPill);
            }

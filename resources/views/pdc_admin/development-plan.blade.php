<x-pdc_admin.layout title="Development Plan - PDC Admin" :user="$user">
    ...
    @php
        ...
        $formAtasanId = old('atasan_id', $prefillData['atasan_id'] ?? null);
        $formStartDate = old('start_date', $prefillData['start_date'] ?? null);
        $formTargetDate = old('target_date', $prefillData['target_date'] ?? null);
    @endphp
    ...
    <x-slot name="scripts">
        <script>
            ...
            const initialTalentRows = @json($initialTalentRows);
            ...
            function handleTalentFilterChange() {
                ...
                loadTalentsByCompany(companyId).then(() => {
            ...
            async function loadTalentsByCompany(companyId) {
                ...
                if (!companyId) {
                    document.querySelectorAll('.talent-select').forEach(s => {
                        s.innerHTML = '<option value="">- Pilih Talent -</option>';
                    });
                ...
                await fetch(`/register/departments?company_id=${companyId}`, {
                    ...
                    .then(data => {
                        ...
                        data.forEach(d => {
                            ...
                            if (String(d.id) === String(oldDeptVal)) {
                                opt.selected = true;
                            }
                ...
                document.querySelectorAll('.talent-select').forEach(sel => {
                    refreshTalentSelect(sel, sel.dataset.selectedTalent || null);
                });
            }

            function refreshTalentSelect(sel, selectedValue = null) {
            ...
            function addTalentRow(prefill = null) {
                ...
                if (prefill?.mentors && prefill.mentors.length > 1) {
                    for (let i = 1; i < prefill.mentors.length; i++) {
                        addMentor(block.querySelector('.btn-add-mentor'));
                    }
                }
                ...
                if (prefill?.mentors) {
                    ...
                    prefill.mentors.forEach((mentorId, index) => {
                        if (mentorSelects[index]) {
                            mentorSelects[index].value = mentorId;
                        }
                    });
                }
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                if (oldCompanyId) {
                    loadTalentsByCompany(oldCompanyId).then(() => {
                        refreshMentorOptions();
                        refreshAtasanOptions();
                    });
                } else {

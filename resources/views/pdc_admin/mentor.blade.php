            ...
            <div class="flex flex-col gap-3 mb-4 max-h-[220px] overflow-y-auto pr-2 custom-scrollbar" style="scrollbar-width: thin;">
                @foreach($mentors as $m)
                    <div class="w-full bg-white border border-[#d1d5db] rounded-lg p-4 flex justify-between items-center transition-colors text-left mentor-card-item" 
                         onclick="handleCardClick({{ $m->id }}, '{{ addslashes($m->nama) }}', '{{ $m->position_id }}', '{{ $m->department_id }}', '{{ addslashes($m->email) }}', '{{ addslashes($m->username) }}', '{{ addslashes($m->remember_token ?? '') }}', this)">
                        <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <button type="button" onclick="togglePassword('input-password', 'eye-icon-mentor')" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400 hover:text-gray-600 transition-colors" tabindex="-1">
                                <svg id="eye-icon-mentor" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">

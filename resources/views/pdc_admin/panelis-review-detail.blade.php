<x-pdc_admin.layout title="Lihat Penilaian Panelis – Individual Development Plan" :user="$user" :hideSidebar="true">
    <x-slot name="styles">
        <style>
            ...
            /* ── Decision Modal ── */
            #decisionModal, #confirmModal { display: none; }
            .decision-card {
                flex: 1;
            ...
            .decision-card:hover { transform: translateY(-3px); box-shadow: 0 8px 24px rgba(0,0,0,0.12); }
            .decision-card.promoted:hover { border-color: #22c55e; }
            .decision-card.not-promoted:hover { border-color: #ef4444; }
            ...
            .decision-card.promoted .card-icon { background: rgba(34,197,94,0.12); }
            .decision-card.not-promoted .card-icon { background: rgba(239,68,68,0.1); }
            .decision-card h4 { font-size: 1rem; font-weight: 800; margin-bottom: 4px; }
            .decision-card.promoted h4 { color: #16a34a; }
            .decision-card.not-promoted h4 { color: #dc2626; }
            .decision-card p { font-size: 0.78rem; color: #64748b; line-height: 1.4; }
    ...
    <div class="flex justify-end gap-3">
        ...
        @if ($isComplete)
            @if ($statusPromo === 'Promoted')
                <button class="btn-selesai" style="background: #22c55e; box-shadow: 0 2px 8px rgba(34,197,94,0.3); cursor: default;" disabled>
            ...
            @elseif ($statusPromo === 'Not Promoted')
                <button class="btn-selesai" style="background: #ef4444; box-shadow: 0 2px 8px rgba(239,68,68,0.3); cursor: default;" disabled>
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                    </svg>
                    Tidak Diangkat ✓
    ...
    <div id="decisionModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" style="background: rgba(15,23,42,0.55); backdrop-filter: blur(4px);">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in">
            ...
            <div class="px-6 pt-6 pb-4">
                <div class="flex items-center justify-between mb-1">
                    ...
                    <button type="button" onclick="closeDecisionModal()" class="text-gray-400 hover:text-gray-600 transition-colors">
                        <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                        </svg>
                    </button>
                </div>
                <p class="text-sm text-[#64748b]">Tentukan hasil promosi untuk <span class="font-semibold text-[#1e293b]">{{ $talent->nama }}</span></p>
            </div>
            {{-- Decision Cards --}}
            ...
            <div class="px-6 py-4 flex justify-end">
                <button type="button" onclick="closeDecisionModal()" class="px-5 py-2 text-sm font-semibold text-gray-700 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors">Batal</button>
            </div>
        </div>
    </div>

    {{-- ── MODAL STEP 2: Konfirmasi ── --}}
    <div id="confirmModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" style="background: rgba(15,23,42,0.55); backdrop-filter: blur(4px);">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            ...
            <div class="mx-6 mb-4 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
                <p class="text-xs text-amber-700 font-medium">⚠️ Tindakan ini tidak dapat dibatalkan setelah dikonfirmasi.</p>
            </div>
            {{-- Footer --}}
            <div class="px-6 py-4 flex gap-3 justify-end border-t border-gray-100">
                <button type="button" onclick="backToDecision()" class="px-5 py-2 text-sm font-semibold text-gray-700 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors">Kembali</button>
                <button type="button" onclick="submitDecision()" id="confirmBtn" class="px-6 py-2 text-sm font-bold text-white rounded-xl transition-colors">Ya, Konfirmasi</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function selectDecision(decision) {
            ...
            if (decision === 'promoted') {
                ...
                confirmDesc.innerHTML = 'Anda akan menetapkan <strong>{{ addslashes($talent->nama) }}</strong> sebagai <strong class="text-green-600">DIANGKAT</strong> ke posisi target.';
                confirmBtn.style.background = '#22c55e';
            } else {
                confirmIcon.innerHTML = '📋';
                confirmIcon.style.background = 'rgba(239,68,68,0.1)';
                confirmTitle.textContent = 'Konfirmasi: Tidak Diangkat';
                confirmDesc.innerHTML = 'Anda akan menetapkan <strong>{{ addslashes($talent->nama) }}</strong> sebagai <strong class="text-red-600">TIDAK DIANGKAT</strong> pada periode ini.';
